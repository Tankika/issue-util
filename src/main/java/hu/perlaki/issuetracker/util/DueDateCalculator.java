...
public class DueDateCalculator 
{
    ...
    public static final int WORK_HOURS_LENGTH = Math.toIntExact(ChronoUnit.HOURS.between(WORK_HOURS_START, WORK_HOURS_END));
    ...
    private void validateSubmitDate(LocalDateTime submitDateTime) {
        ...
        if(!isWorkDay(submitDayOfWeek)) {
            throw new IllegalArgumentException(
                    String.format("Submit date must be on a work day. First work day is: %s, last work day is: %s", WORK_DAYS_START, WORK_DAYS_END));
        } else if(submitTime.isBefore(WORK_HOURS_START) || submitTime.isAfter(WORK_HOURS_END)) {
            throw new IllegalArgumentException(
                    String.format("Submit time must be in a working hour. Work hours start: %s, work hours end: %s", WORK_HOURS_START, WORK_HOURS_END));
        }
    }
    ...
    private void validateTurnaroundTime(int turnaroundTime) {
        if(turnaroundTime <= 0) {
            throw new IllegalArgumentException(
                    String.format("Turnaround time must be positive"));
        }
    }
